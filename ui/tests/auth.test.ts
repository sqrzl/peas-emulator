...
describe('admin authentication', () => {
  ...
  it('treats unauthorized resolution as signed out and guards app routes', async () => {
    installAuthMock(401);
    ...
    try {
      ...
      const app = manifest.records.find((record) => record.path === '/app');
      const login = manifest.records.find((record) => record.path === '/auth');
      const logout = manifest.records.find(
        (record) => record.path === '/auth/logout'
      );

      expect(resolved.session).toBe(null);
      expect(app?.options.policies?.length).toBeGreaterThan(0);
      expect(login?.options.policies?.length).toBeGreaterThan(0);
