...
function rowButton(root: HTMLElement, rowText: string): HTMLButtonElement | undefined {
...
describe("simplified page flows", () => {
  ...
  it("renders login and shows auth errors after submit", async () => {
    globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === "/admin/v1/auth/login") {
        return jsonResponse({ code: "Unauthorized", error: "Bad credentials" }, 401);
      }
  ...
  it("renders buckets and creates a bucket from the home dialog", async () => {
    ...
    try {
      ...
      const addButton = Array.from(root.querySelectorAll("button")).find((button) =>
        button.textContent?.includes("Add bucket")
      );
      click(addButton!);
      await flush();
  ...
  it("renders bucket search, pagination controls, and delete acknowledgement", async () => {
    ...
    globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        ...
        bucketObjects = bucketObjects.filter((object) => object.key !== "one.txt");
      ...
      if (
        ...
        bucketObjects = bucketObjects.filter((object) => object.key !== "two.txt");
        return new Response(null, { status: 204 });
      }
    ...
    try {
      ...
      const searchInput = root.querySelector("#bucket-search") as HTMLInputElement;
      ...
      const confirmDelete = Array.from(document.querySelectorAll("button")).find((button) =>
  ...
  it("renders blobs and uploads a blob from the bucket dialog", async () => {
    ...
    globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        url.pathname === "/admin/v1/buckets/alpha/objects/docs%2Freadme.txt/content" &&
        request.method === "PUT"
      ) {
        uploaded = true;
        return jsonResponse(
    ...
    try {
      ...
      const addButton = Array.from(root.querySelectorAll("button")).find((button) =>
        button.textContent?.includes("Add blob")
      );
      click(addButton!);
      await flush();

      const keyInput = document.querySelector("#blob-key") as HTMLInputElement;
      const fileInput = document.querySelector("#blob-file") as HTMLInputElement;
      const form = document.querySelector("form") as HTMLFormElement;
  ...
  it("renders blob search, pagination controls, richer sizes, and delete", async () => {
    ...
    try {
      ...
      click(deleteButton!);
      await flush();
      expect(document.body.textContent).toContain("Delete image.png from alpha.");
  ...
  it("renders blob metadata", async () => {
    globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === "/admin/v1/buckets/alpha/objects" && request.method === "GET") {
        return jsonResponse({
          items: [
            {
              key: "docs/readme.txt",
  ...
  it("downloads a blob from the detail page", async () => {
    ...
    try {
      ...
      const downloadButton = Array.from(root.querySelectorAll("button")).find((button) =>
        button.textContent?.includes("Download blob")
      );
