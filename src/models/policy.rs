...
impl Acl {
	...
	pub fn to_grants(&self, owner_id: &str) -> Vec<Grant> {
		...
		match self.canned {
			...
			CannedAcl::BucketOwnerRead | CannedAcl::BucketOwnerFullControl => vec![
				Grant {
					grantee: Grantee::CanonicalUser {
						id: owner_id.to_string(),
						display_name: None,
...
impl Authorizer {
	...
	fn permission_covers_action(permission: &Permission, action: &str) -> bool {
		match permission {
			...
			Permission::Read => {
				action.contains("GetObject") || action.contains("ListBucket")
			}
			Permission::Write => {
				action.contains("PutObject") || action.contains("DeleteObject")
			}
			Permission::ReadAcp => action.contains("GetObjectAcl") || action.contains("GetBucketAcl"),
			Permission::WriteAcp => action.contains("PutObjectAcl") || action.contains("PutBucketAcl"),
		}
	}
	...
	fn resource_matches(resources: &ResourceList, context_resource: &str) -> bool {
		...
		match resources {
			...
			ResourceList::Multiple(resource_list) => resource_list.iter().any(|r| check_resource(r)),
		}
	}
}
